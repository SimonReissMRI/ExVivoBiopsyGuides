function V = SurfaceSmooth(Vertices, Faces, VoxSize, ViewPoint, DisplTol, IterTol, Freedom, Verbose)
...
% With above these improvements, the code is now more general to use.

%% Improved by Chong Wu, City University of Hong Kong, Hong Kong SAR, China, 2018.10.4
%
% Improved the normal calculation, revised the sparse error, and NaN error.
% The original work by Marc Lalancette, Toronto, Canada, 2014-02-04 is: 
% https://ww2.mathworks.cn/matlabcentral/fileexchange/45416-surfacesmooth?s_tid=srchtitle

%% The work done by Marc Lalancette, Toronto, Canada, 2014-02-04
...
%% Introduced by Chong Wu, 2018.10.4
% Help to make the normal direction consistent.
if ~exist('ViewPoint', 'var') || isempty(ViewPoint)
    ViewPoint = [0,0,0];
end
...
if Verbose
    [~, ~, FNdA, FdA] = VertexNormals(Vertices,ViewPoint);
...
%% Improved by Chong Wu, 2018.10.3 
% Make every edge distinct in one direction
t = sort(Faces,2);
...
% This expression works because each edge is found once in each
% direction, as long as all normals are consistently pointing in (or
% out).

% construct every edge is distinct in one direction
...
while LastMaxDispl(1) > DisplTol(1) && LastMaxDispl(2) > DisplTol(2) && ...
        Iter < IterTol
    Iter = Iter + 1;
    [N, VdA] = VertexNormals(V,ViewPoint);
...
if Verbose && IterTol > 0
    [~, ~, FNdA, FdA] = VertexNormals(V,ViewPoint);
...
% ----------------------------------------------------------------------
%% Improved by Chong Wu, 2018.10.4
% New normal calculation function by introducing ViewPoint to control the
% normals' direction.
% Revise an error which the original function will get in trouble when some vertices
% are not in faces.
%
% Calculate dA normal vectors to each vertex. 

    function [N, VdA, FNdA, FdA] = VertexNormals(V,ViewPoint)
        N = zeros(nV, 3);
        % Get face normal vectors with length the size of the face area.
        FNdA = CrossProduct( (V(Faces(:, 2), :) - V(Faces(:, 1), :)), ...
            (V(Faces(:, 3), :) - V(Faces(:, 2), :)) ) / 2;
        % Determine whether the normals' direction are consistent to ViewPoint.
        nfnda = sum(FNdA.*(ViewPoint - Faces(:, 2)),2);
        loc = find(nfnda <0);
        ...
        % Find NaN value and replace it with 0.
        N(isnan(N)) = 0;
    end
